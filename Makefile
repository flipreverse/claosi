...
EVAL_RELAY_READER=evalreader
EVAL_RELAY_READER_SRC = evalreader.c
EVAL_RELAY_READER_OBJ=$(patsubst %.o,$(BUILD_USER)/$(TEST_DIR)/%.o,$(EVAL_RELAY_READER_SRC:%.c=%.o))

HASH_TEST=hash-test
HASH_TEST_SRC = hash-test.c dummy.c
HASH_TEST_OBJ=$(patsubst %.o,$(BUILD_USER)/$(TEST_DIR)/%.o,$(HASH_TEST_SRC:%.c=%.o))
#*****************************			END SOURCE FILE				*****************************
...
# Example: $(<name>_OBJ)
TEST_OBJ = $(QUERY_TEST_OBJ) $(DATAMODEL_TEST_OBJ) $(RESULTSET_TEST_OBJ) $(OBJ_API_TEST_OBJ) $(EVT_API_TEST_OBJ) $(EVAL_RELAY_READER_OBJ) $(HASH_TEST_OBJ)
TEST_BIN = $(QUERY_TEST) $(DATAMODEL_TEST) $(RESULTSET_TEST) $(OBJ_API_TEST) $(EVT_API_TEST) $(EVAL_RELAY_READER) $(HASH_TEST)
...
$(BUILD_PATH)/$(EVAL_RELAY_READER): $(EVAL_RELAY_READER_OBJ)
	@echo $(LD_TEXT)
	$(OUTPUT)$(CC) $^ $(LDFLAGS) $(LDLIBS) -o $@

$(BUILD_PATH)/$(HASH_TEST): $(HASH_TEST_OBJ) $(LIB_COMMON_OBJ) $(LIB_USERSPACE_OBJ)
	@echo $(LD_TEXT)
	$(OUTPUT)$(CC) $^ $(LDFLAGS) $(LDLIBS) -o $@
